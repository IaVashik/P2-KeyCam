...
function SetCameraPosition(mode) // mode - true - линейное движение; false - плавное движение
{
    ...
    {
        if(mode) { //ЛИНЕЙНОЕ ДВИЖЕНИЕ КАМЕРЫ
            ...
            SetAnglesV(camera, lerpVector(keypoints[currentKey].angles, keypoints[currentKey+1].angles,progress) ) // линейно интерполировать угол камеры между двумя точками
        ...
        else { //SMOOTH ДВИЖЕНИЕ КАМЕРЫ
            camera.SetOrigin( lerpVector(camera.GetOrigin(), keypoints[currentKey+1].origin,0.01) ) // плавно приближаться к следующей точке по позиции
            SetAnglesV(camera, lerpVector(camera.GetAngles(), keypoints[currentKey+1].angles,0.01) ) // плавно приближаться к следующей точке по углу
            if(TraceGetDist(camera.GetOrigin(),keypoints[currentKey+1].origin)<1) currentKey++ // если расстояние до следующей точки меньше 1, перейти к ней
        }
